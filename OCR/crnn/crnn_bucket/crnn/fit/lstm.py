...
def _lstm_unroll_base(seq_len,num_lstm_layer, num_hidden):
    ...
    channel,height,width = (1,30,seq_len*8)


    print("model give seq_len:",seq_len)
    ...
    print("lstm_fc shape:", pred_fc.infer_shape(data=(128,1,30,seq_len*8),l0_init_h=(128,100),l1_init_h=(128,100))[1][0])
    return pred_fc


def lstm_unroll(seq_len,num_lstm_layer, num_hidden, num_label, loss_type=None):
    """
    ...
    pred = _lstm_unroll_base(seq_len,num_lstm_layer, num_hidden)
    #pdb.set_trace()

    if loss_type:
        # Training mode, add loss
        return ctc_loss._add_ctc_loss(pred, seq_len, num_label, loss_type)
    else:
...
def sym_gen(seq):
    hp = Hyperparams()
    sym = lstm_unroll(seq_len=seq,
                      num_lstm_layer=hp.num_lstm_layer,
                      num_hidden=hp.num_hidden,
                      num_label=hp.num_label,
                      loss_type='ctc')
    return sym,('data', 'l0_init_c', 'l1_init_c', 'l0_init_h', 'l1_init_h',),('label',)
